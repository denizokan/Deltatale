...
class BattleDialogue:
    """This class is used for rendering text in battles."""
    ...
    def init_text(self, page_data):
        """Initializes text canvas object."""
        pos = page_data["pos"]
        if pos == "bottom":
            x1, x2 = 22, self.game.constants.WIDTH - 22
        ...
        if page_data["face"] != None:
            ...
            self.portrait_id = self.game.canvas.create_image(x1, y1 + 17, image=self.portrait_sprite, anchor="nw")
            self.active_text_elements.append(self.portrait_id)
            self.text_coords = (x1 + 120, y1 + 13)
        else:
    ...
    def start_dialogue(self, text, interaction_index=0, actors=None, on_complete=None):
        ...
        if self.is_active: raise RuntimeError("Cannot start a new battle dialogue because another one is already being shown.")
        ...
        if self.typewriter_timer <= 0:
            ...
            while self.tokenized_text[self.token_index]["type"] != "char":
                ...
                if token["type"] == "color":
                    ...
                    self.cursor_y += 1
                    self.text_coords = (self.og_text_coords[0], self.og_text_coords[1] + (self.cursor_y * 30))
            ...
            if self.tokenized_text[self.token_index]["type"] == "char":
                ...
                fixed_char_width = 15 
                self.text_coords = (x + fixed_char_width, self.og_text_coords[1] + (self.cursor_y * 30))
